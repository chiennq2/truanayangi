        ...
        <a href={`${basePath}/`} className="brand">
          <picture>
            <source
              ...
              srcSet={`${basePath}/brand/icon-cs-v2.webp`}
            />
            <img
              className="brand-logo"
              src={`${basePath}/brand/logo-cs-v2.webp`}
              width={180}
              height={60}
              alt="Trưa Nay Ăn Gì"
              fetchPriority="high"
        ...
        <>
          ...
          {!eligible.length && (
            <p className="preferences-message">
              {language === "vi"
                ? "Pool không có món phù hợp. Tắt bộ lọc chay hoặc thêm món."
          ...
          {counterEnabled && (
            <p
              ...
              title={
                language === "vi"
                  ? "Lượt mở trên trình duyệt này, lưu bằng cookie"
                  : "Spins on this browser, stored in cookies"
              }
            >
              {language === "vi" ? "Bạn đã mở" : "You have opened"}{" "}
              <strong>
                {localSpins === null
                  ? "—"
          ...
          <section className="case-panel" aria-label={t.caseLabel}>
            <div
              className={`reel-window ${moving ? "is-spinning" : ""} `}
              ref={viewport}
            >
